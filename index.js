...
class ServicoCalculoFatura {
    ...
    calcularTotalApresentacao(apre) {
        ...
        switch (this.repo.getPeca(apre).tipo) {
            case "tragedia": total = 40000;
                if (apre.audiencia > 30) {
                    total += 1000 * (apre.audiencia - 30);
                }
                break;
            case "comedia": total = 30000;
                ...
                break;
            default:
                throw new Error(`Peça desconhecia: ${
                    this.repo.getPeca(apre).tipo
                }`);
        }
...
function gerarFaturaStr(fatura, calc) {
    ...
    faturaStr += `Valor total: ${
        formatarMoeda(calc.calcularTotalFatura(fatura.apresentacoes))
    }\n`;
...
//     for (let apre of fatura.apresentacoes) {
//         faturaHTML += '<li> ' + calc.repo.getPeca(pecas, apre).nome + ': ' + formatarMoeda(calc.calcularTotalApresentacao(pecas, apre)) + ' (' + apre.audiencia + ' assentos) </li>\n';
//     }
